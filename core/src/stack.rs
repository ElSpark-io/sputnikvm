...
impl<M: VMApi> Stack<M> {
	...
	#[inline]
	/// Stack data.
	pub fn data(&self) -> &ManagedVec<M, EH256<M>> {
		&self.data
	}
	...
	pub fn pop(&mut self) -> Result<EH256<M>, ExitError> {
	...
	pub fn push(&mut self, value: EH256<M>) -> Result<(), ExitError> {
	...
	pub fn peek(&self, no_from_top: usize) -> Result<EH256<M>, ExitError> {
	...
	/// `StackError::Underflow` is returned.
	pub fn set(&mut self, no_from_top: usize, val: EH256<M>) -> Result<(), ExitError> {
		if self.data.len() > no_from_top {
