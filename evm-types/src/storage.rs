...
#[allow(dead_code)]
pub fn address_to_key<M: VMApi>(prefix: KeyPrefix, address: &ETHAddress<M>) -> [u8; 22] {
    let mut result = [0u8; 22];
    result[0] = u8::from(VersionPrefix::V1);
    result[1] = u8::from(prefix);
    result[2..22].copy_from_slice(&address.0.to_byte_array());
    result
}
...
#[allow(dead_code)]
fn normal_storage_key<M: VMApi>(address: &ETHAddress<M>, key: &H256) -> [u8; 54] {
...
#[allow(dead_code)]
fn generation_storage_key<M: VMApi>(
    address: &ETHAddress<M>,
    key: &H256,
    generation: u32,
) -> [u8; 58] {
